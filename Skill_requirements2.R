...
#Corrections for wrongly assigned types based on title
...
get_top_skills <- function(skills_data){
  ...
  desc_data = tolower(skills_data[,6]) 
  desc_data = gsub("\n", " ",desc_data)
  desc_data = str_replace_all(desc_data, "[^[:alnum:]]", " ")
  
  word_lists = (str_split(desc_data, " ", simplify = FALSE))
  unique_word_lists = sapply(word_lists, function(z) unique(z))

  full_word_list = do.call(c, unique_word_lists)
  ...
  if("spark" %in% word_count$Var1 & "apache" %in% word_count$Var1){
    word_count[word_count$Var1 == "spark",2] = 
    word_count[word_count$Var1 == "spark",2] +
    word_count[word_count$Var1 == "apache",2] - 
    sum(grepl("apache spark",desc_data))
  }
  
  word_count <- word_count %>% arrange(desc(word_count[,2]))
  word_count[,1] = str_to_title(word_count[,1])
  ...
  word_count[,2] = (word_count[,2]/dim(skills_data)[1])*100
  return(word_count)
}
...
SE <- map_data("world") %>% filter(region=="Sweden")
data <- read.csv("se.csv") #from https://simplemaps.com/data/se-cities
data <- merge(loc_count,data,by.x = "loc", by.y = "city")
